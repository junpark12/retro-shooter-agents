...
출력 디렉터리는 새 경로를 사용합니다. `--idle-seconds 100`으로 사망/종료 화면을
기다리는 시간을 지정할 수 있습니다. `--baseline`은 원본 실행 파일에서 Enter를
길게 눌렀을 때 기체 선택을 건너뛰는 현상만 확인하는 비교 모드입니다.

### 실제 입력으로 확인한 범위

WSL의 격리된 Xvfb 창에서 12개 검사를 통과하고 47개 캡처를 저장했습니다.
Enter를 길게 눌러도 기체 선택 화면이 유지되며, 새 입력으로 기체를 바꾸고
게임을 시작했습니다. 이동·사격 화면과 폭탄 섬광을 확인했고, P를 길게 눌러도
일시정지가 유지되었습니다. 다른 창으로 포커스를 옮겼다가 돌아와도 정지
상태가 유지되고, 새 P 입력으로 재개되었습니다.

정지 상태가 아닌 게임을 그대로 진행해 자연스럽게 GAME OVER에 도달한 뒤,
Enter로 타이틀 → 기체 선택 → 새 전투에 다시 진입했습니다.
차지와 Z 키 입력도 실행했지만 **해당 공격의 정확한 효과와 Continue 화면은
이 창 입력 검사에서 확인하지 않았습니다.** 5개 스테이지/보스의 전체 진행은
앞서 설명한 결정적 픽스처 검사이며, 자연 플레이 클리어와 구분합니다.
